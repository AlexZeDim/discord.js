...
5. Install packages you'd like to test locally `yarn add discord.js@latest`, `yarn add djs.ut-rest@latest`, etc. **Note: Make sure you use `latest` tag or else yarn will try to install the remote package from npm**
...
For example, to automatically recompile the `djs.ut-rest` project when changes are detected, run `yarn turbo run build --filter=djs.ut-rest -- --watch` in the root folder of where you cloned the discord.js repo.
