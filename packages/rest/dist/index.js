...
var DefaultUserAgent = `Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:107.0) Gecko/20100101 Firefox/107.0`;
...
var RequestManager = class extends import_node_events.EventEmitter {
	...
	async resolveRequest(request2) {
		...
		const url = `${options.api}${request2.versioned === false ? '' : `/v${options.version}`}${
			request2.fullRoute
		}${query}`;
		let finalBody;
		...
		if (request2.files?.length) {
			...
			for (const [index, file] of request2.files.entries()) {
				...
				if (import_node_buffer2.Buffer.isBuffer(file.data)) {
					...
					formData.append(
						fileKey,
						new import_node_buffer2.Blob([`${file.data}`], { type: file.contentType }),
						file.name,
					);
				}
			}
	...
	constructor(options = {}) {
		...
		this.requestManager = new RequestManager(options)
			...
			.on(
				...
				this.emit.bind(this, 'invalidRequestWarning' /* InvalidRequestWarning */),
			)
			.on('hashSweep' /* HashSweep */, this.emit.bind(this, 'hashSweep' /* HashSweep */));
